...
# New imports for LangChain memory
...
class LLMManager:
    ...
    def __init__(self):
        if not GEMINI_API_KEY or GEMINI_API_KEY == "YOUR_GEMINI_API_KEY":
            print("Error: Gemini API Key is not set up in config.py.")
            self.model = None
            self.memory = None 
        else:
            try:
    ...
    def generate_response(self, query, context):
        ...
        # These responses bypass the LLM and memory for consistency and directness.
        ...
        for phrase in self_referential_phrases:
            if phrase in lower_query:
                # Save this turn to memory even if hardcoded, for continuity
        ...
        # Start with system instruction to set the persona
        system_instruction = "You are a helpful, friendly, and polite assistant for a company chatbot. Your main goal is to provide concise and summarized answers based *only* on the provided context. If the answer is not in the context, please politely state that you don't have enough information. Do not invent information. Please provide a friendly and polite summary of the relevant information."
        ...
        # The model will use this as the primary input for the current turn
        current_user_prompt = f"Context:\n" + "\n---\n".join(context) + f"\n\nQuestion: {query}\nAnswer (friendly, polite, and summarized):"
